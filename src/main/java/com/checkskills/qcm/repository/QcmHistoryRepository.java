...
public interface QcmHistoryRepository  extends JpaRepository<QcmHistory, Long> {
    ...
    @Query("SELECT new com.checkskills.qcm.model.custom.QcmHistoryOrder(qcm_history.id, qcm.id, qcm_history.code, qcm_history.dateBought, qcm_history.dateUsed, qcm_history.dateInvited, qcm.title, qcm_history.candidate_name, qcm_history.candidate_mail, qcm_history.success, qcm_history.status)" + "from QcmHistory as qcm_history LEFT JOIN Qcm as qcm ON qcm_history.qcm.id = qcm.id WHERE qcm_history.employer.id = :employer_id ORDER BY qcm_history.dateUsed DESC, qcm_history.dateBought DESC")
    List<QcmHistoryOrder> findQcmHistoryOrder(@Param("employer_id") Long employer_id);

    @Query("SELECT new com.checkskills.qcm.model.custom.QcmHistoryOrder(qcm_history.id, qcm.id, qcm_history.code, qcm_history.dateBought, qcm_history.dateUsed, qcm_history.dateInvited, qcm.title, qcm_history.candidate_name, qcm_history.candidate_mail, qcm_history.success, qcm_history.status)" + "from QcmHistory as qcm_history LEFT JOIN Qcm as qcm ON qcm_history.qcm.id = qcm.id WHERE qcm_history.employer.id = :employer_id AND qcm_history.qcm.id = :qcm_id")
